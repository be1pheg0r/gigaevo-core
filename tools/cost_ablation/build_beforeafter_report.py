#!/usr/bin/env python3
"""Before/after report for the 2026-07-31 prediction-math fix.

Same output shape as build_report.py (LaTeX table + PNG table + error-decay
plots), but the two compared conditions are the OLD math and the NEW math on
the SAME logs, instead of with-agent/without-agent on different runs:

  before — the ``[CostMonitorHookJSON]`` lines the log itself recorded, i.e.
           what the shipped estimator predicted while the run was happening
  after  — the same raw events replayed through the currently-installed
           estimator (tools/cost_ablation/replay_from_log.py)

Plus two diagnostic figures for the root causes:
  concurrency_diagnosis.png — measured achieved LLM concurrency vs the
           hardcoded max_in_flight divisor, and the duration error each
           divisor implies
  token_coverage.png — residual token error after the fix vs the share of
           tokens the LLM_CALL event stream actually carries
  agent_activity.png — when the agent is awake under the old accepted-mutant
           tick vs the new confidence-interval-breach trigger

Usage:
    python3 tools/cost_ablation/build_beforeafter_report.py LOG... --out-dir DIR
"""
...
ADJ_LINE_RE = re.compile(r"\[CostMonitorHook\] adjustments: cold=(\S+) golden=(\S+) growth=(\S+)")
...
def parse_before(path: Path) -> dict | None:
    ...
    return dict(
        ...
        event_tokens=ev_tokens, coverage_pct=ev_tokens / actual_tokens * 100,
        llm_service_s=llm_service_s, llm_span_s=span, max_in_flight=max_in_flight,
        attempts=attempts, agent_calls=agent_calls, agent_effective=agent_effective,
...
def build_table_png(rows: dict[str, dict[str, dict]], out: Path, pred_at: int) -> None:
    ...
    body = [["Task", "Model", f"Pred tok @{pred_at}", "Act tok", "Tok err %",
             f"Pred dur @{pred_at} (s)", "Act dur (s)", "Dur err %"]]
    colors = [None]
    for key, conds in rows.items():
        first = True
        n_att = conds["agent"]["n_points"]
        for cond in CONDS:
            ...
            body.append([
                ...
                f"{pt:,.0f}", f"{s['actual_tokens']:,}", f"{te:+.1f}",
                f"{pd:.0f}", f"{s['actual_duration']:.0f}", f"{de:+.1f}",
            ])
...
def build_error_decay_png(rows: dict[str, dict[str, dict]], out: Path, field: str,
    ...
    for ax, (key, conds) in zip(flat, rows.items()):
        ...
        for cond in CONDS:
            s = conds[cond]
            actual = (s["actual_duration"] if field == "predicted_duration_s"
                      else s["actual_tokens"])
            xs = np.arange(1, len(s["series"]) + 1)
            errs = [abs(p[field] - actual) / actual * 100 for p in s["series"]]
            ax.plot(xs, errs, color=COND_COLOR[cond], linewidth=1.8,
        ...
        ax.set_title(f"{key}  — {n_adj} agent adjustment{'s' if n_adj != 1 else ''}",
...
def build_agent_activity_png(rows: dict[str, dict[str, dict]], out: Path) -> None:
    ...
    for i, k in enumerate(keys):
        ...
        ax.hlines(y - 0.16, 1, n, color="#e2e4e8", linewidth=7, zorder=1)
        ax.scatter(old_all, [y + 0.16] * len(old_all), s=42, zorder=3,
                   color=["#c0504d" if a in old_eff else "#8b96a3" for a in old_all],
                   marker="|", linewidths=2.4)
        ax.scatter(new, [y - 0.16] * len(new), s=42, color="#2a9d5c", marker="|",
                   linewidths=2.4, zorder=3)
        ax.text(-2, y + 0.16, "every 5 accepted", ha="right", va="center", fontsize=8,
                color="#666")
        ax.text(-2, y - 0.16, "CI breach", ha="right", va="center", fontsize=8,
                color="#2a6b45", fontweight="bold")
        ax.text(-2, y + 0.52, k, ha="right", va="center", fontsize=9, fontweight="bold")
        ax.text(n + 2, y + 0.16, f"{len(old_all)}", va="center", fontsize=8, color="#666")
        ax.text(n + 2, y - 0.16, f"{len(new)}", va="center", fontsize=8,
                color="#2a6b45", fontweight="bold")
    ...
    handles = [
        plt.Line2D([], [], color="#8b96a3", marker="|", linestyle="", markersize=9,
                   markeredgewidth=2.4, label="woken, no change made"),
        plt.Line2D([], [], color="#c0504d", marker="|", linestyle="", markersize=9,
                   markeredgewidth=2.4, label="woken, multiplier moved"),
        plt.Line2D([], [], color="#2a9d5c", marker="|", linestyle="", markersize=9,
                   markeredgewidth=2.4, label="new trigger would wake it"),
    ]
    ax.legend(handles=handles, fontsize=8, ncol=3, loc="upper center",
              bbox_to_anchor=(0.5, 1.02), frameon=False)
    ax.set_title("When the agent is awake: accepted-mutant tick vs surprise trigger",
...
def build_concurrency_png(rows: dict[str, dict[str, dict]], out: Path) -> None:
    ...
    err_maxif = [
        (rows[k]["diag"]["llm_service_s"] / rows[k]["diag"]["max_in_flight"]
         - rows[k]["diag"]["actual_duration"]) / rows[k]["diag"]["actual_duration"] * 100
        for k in keys
    ]
    y = np.arange(len(keys))
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 0.55 * len(keys) + 3))

    ax1.barh(y, conc, color=["#b03030" if c < m else "#2a6fb0" for c, m in zip(conc, maxif)])
    ax1.axvline(8, color="#333", linestyle="--", linewidth=1.5, label="max_in_flight = 8 (the divisor used)")
    ax1.set_yticks(y); ax1.set_yticklabels(keys, fontsize=9)
    ax1.invert_yaxis()
    ax1.set_xlabel("LLM calls actually running in parallel")
    ax1.set_title("Measured concurrency vs the hardcoded divisor", fontsize=11, fontweight="bold")
    ax1.legend(fontsize=8, loc="lower right")
    ax1.grid(alpha=0.3, axis="x")
    for i, c in enumerate(conc):
        ax1.text(c + 0.15, i, f"{c:.1f}", va="center", fontsize=8)

    ax2.barh(y, err_maxif, color=["#b03030" if abs(e) > 15 else "#c9a227" for e in err_maxif])
    ...
    ax2.set_title("...which reproduces the whole observed error spread",
                  fontsize=11, fontweight="bold")
    ax2.grid(alpha=0.3, axis="x")
    for i, e in enumerate(err_maxif):
        ax2.text(e + (2 if e >= 0 else -2), i, f"{e:+.0f}%", va="center",
                 ha="left" if e >= 0 else "right", fontsize=8)

    fig.suptitle("Root cause of the duration error: Little's Law divided by the wrong number",
                 fontsize=13)
...
def build_coverage_png(rows: dict[str, dict[str, dict]], out: Path) -> None:
    ...
    ax.plot([0, lim], [0, lim], color="#999", linestyle="--", linewidth=1.2,
    ...
    ax.set_title("After anchoring, the only token error left\nis the telemetry the estimator never saw",
...
def main() -> None:
    ...
    ap.add_argument("--pred-at", type=int, default=10,
    ...
    for path in args.logs:
        ...
        print(f"{path.stem:34s} @{args.pred_at:<3d} "
    ...
    build_error_decay_png(rows, args.out_dir / "error_decay_duration.png",
                          "predicted_duration_s", "|duration pred error|, %",
                          "Duration prediction error as the run proceeds: auto vs auto + agent")
    build_error_decay_png(rows, args.out_dir / "error_decay_tokens.png",
                          "predicted_tokens", "|token pred error|, %",
