...
def test_next_redis_db_cycles_through_pool():
    ...
    seen = [app_module._next_redis_db() for _ in range(len(app_module.REDIS_DB_POOL) + 1)]
...
def test_job_public_dict_hides_private_fields_and_serializes_points():
    ...
    job.points.append(app_module.MutantPoint(
        mutant=1, predicted_tokens=100, token_ci_low=80, token_ci_high=120,
        predicted_duration_s=10.0, ci_low_s=8.0, ci_high_s=12.0,
        elapsed_s=5.0, actual_tokens_so_far=90,
    ))
    d = job.public_dict()
